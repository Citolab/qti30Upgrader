...
<!--
     ...
     NOTES:
     This XSL transformation requires a XSL 3.0 capable processor and has been developed/tested using
the Saxon-HE
     Java implementation version 9.9.1.7.
     
     This XSL transformation is incomplete.  It only supports assessmentItem documents at this point, and
does
     not purport to support these completely.  It is intended as a starting point for those who are
venturing
     ...
     * Content of feedbackBlock, modalFeedback, rubricBlock and templateBlock is reordered and
encapsulated
     in qti-content-body.
...
<xsl:stylesheet xmlns:xi="http://www.w3.org/2001/XInclude"
    ...
    xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:mml3="http://www.w3.org/2010/Math/MathML"
    xmlns:imx="http://ets.org/imex"
    ...
    <xsl:variable
        name="qti3NamespaceUri" select="'http://www.imsglobal.org/xsd/imsqtiasi_v3p0'" />
    <xsl:variable
        name="qti3RptemplatesUri" select="'https://purl.imsglobal.org/spec/qti/v3p0/rptemplates/'" />
    <xsl:variable
        name="mmlNamespaceUri" select="'http://www.w3.org/1998/Math/MathML'" />
    <xsl:variable
        name="ssmlNamespaceUri" select="'http://www.w3.org/2001/10/synthesis'" />
    ...
    <xsl:template
        ...
        <xsl:if test="$addSchematronPi">
            ...
            <xsl:processing-instruction
                name="xml-model">href="https://purl.imsglobal.org/spec/qti/v3p0/schema/xsd/imsqti_asiv3p0_v1p0.xsd" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron"</xsl:processing-instruction>
        </xsl:if>
        <xsl:text>&#10;</xsl:text>
        <xsl:apply-templates />
    </xsl:template>
    ...
    <xsl:template
        match="comment() | processing-instruction()[not(name() = 'xml-model' and matches(., 'schematypens=&quot;http://purl.oclc.org/dsdl/schematron&quot;'))]">
        <xsl:copy />
    </xsl:template>
    ...
    <xsl:template
        match="@*">
        <xsl:attribute
            name="{if (matches(name(), '^(aria|data)-')) then name() else imx:kabobize(name(), '')}"
    ...
    <!-- QTI assessment specific element names get qti-kabobified... -->
    <xsl:template
        match="*:assessmentTest | *:testPart | *:assessmentSection | *:assessmentSectionRef | *:assessmentItemRef | *:weight | *:outcomeProcessing | *:outcomeCondition | *:outcomeIf | *:outcomeElse | *:testVariables | *:timeLimits | *:itemSessionControl | *:selection | *:ordering | *:adaptiveSelection | *:adaptiveEngineRef | *:adaptiveSettingsRef | *:metadataRef | *:branchRule | *:preCondition">
        <xsl:element name="{imx:qti-kabobify(name())}" namespace="{$qti3NamespaceUri}">
            <xsl:apply-templates select="@* | node()" />
        </xsl:element>
    </xsl:template>
    ...
    <!-- Stripping out APIP... -->
    <xsl:template
        match="apip:apipAccessibility" />


    <!-- Inject Schematron association PI and schemaLocation... -->
    <xsl:template
        match="*:assessmentItem | *:assessmentStimulus | *:assessmentTest">
        <xsl:element name="{imx:qti-kabobify(name())}" namespace="{$qti3NamespaceUri}">
            ...
            <xsl:apply-templates
                select="@* except @xsi:schemaLocation | node()" />
        </xsl:element>
    </xsl:template>
    
    <xsl:template
        match="*:feedbackBlock | *:modalFeedback | *:rubricBlock | *:templateBlock">
        <xsl:element name="{imx:qti-kabobify(name())}" namespace="{$qti3NamespaceUri}">
            <xsl:apply-templates select="@* | *:stylesheet" />
            <!-- Adding qti-content-body ... -->
            <xsl:element name="qti-content-body"
                namespace="{$qti3NamespaceUri}">
    ...
    <xsl:template
        match="*:responseProcessing/@template">
        <xsl:attribute name="{name()}"
            select="concat(replace(., '^.*/', $qti3RptemplatesUri), '.xml')" />
    </xsl:template>
    
    <xsl:function
        name="imx:qti-kabobify" as="xs:string">
        <xsl:param name="s" as="xs:string" />
        <xsl:value-of
            select="concat('qti-', imx:kabobize($s, ''))" />
    </xsl:function>
    
    <xsl:function
        name="imx:kabobize" as="xs:string">
        <xsl:param name="s" as="xs:string" />
        <xsl:param name="a" as="xs:string" />
        <xsl:if
            test="substring($s, 1, 1)">
            <xsl:if test="matches($s, '^[A-Z]')">
                <xsl:value-of
                    select="imx:kabobize(substring($s, 2), concat($a, '-', lower-case(substring($s, 1, 1))))" />
            </xsl:if>
            <xsl:if
                test="not(matches($s, '^[A-Z]'))">
                <xsl:value-of
                    select="imx:kabobize(substring($s, 2), concat($a, substring($s, 1, 1)))" />
            </xsl:if>
        </xsl:if>
        <xsl:if
            test="not(substring($s, 1, 1))">
